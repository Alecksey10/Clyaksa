...
class FloatField(FieldStricts):
    
    def __init__(self, strcits_dict:dict=None, *args, **kwargs):
        if(strcits_dict == None):
            strcits_dict = {}
        
        self.default =  strcits_dict.get("default", 0.0)
        self.max = strcits_dict.get("max", 1000000.0)
        self.min = strcits_dict.get("min", -1000000.0)
        self.name = strcits_dict.get("name", f"{random.randint(0,1000000)}")
        self.step = strcits_dict.get("step", 0.001)
        self.order = -1
